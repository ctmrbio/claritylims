...
class Extension(BaseCreateSamplesExtension):
    ...
    def create_in_mem_container(self, samplesheet, container_specifier, 
        ...
        for _, row in samplesheet.csv.iterrows():
            ...
            substance.udf_map.force("Ct_1", ct_values[0] if ct_values[0] else -1.0)
            substance.udf_map.force("Ct_2", ct_values[1] if ct_values[1] else -1.0)
            substance.udf_map.force("Ct_3", ct_values[2] if ct_values[2] else -1.0)
            substance.udf_map.force("Ct_4", ct_values[3] if ct_values[3] else -1.0)
            substance.udf_map.force("Ct_5", ct_values[4] if ct_values[4] else -1.0)
